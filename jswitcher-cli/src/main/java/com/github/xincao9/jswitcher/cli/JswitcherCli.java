...
public class JswitcherCli {
    ...
    public static void main(String... args) throws Throwable {
        Terminal terminal = TerminalBuilder.builder()
                .system(true)
                .build();
        Completer completer = new ArgumentCompleter(new StringsCompleter("check", "on", "off", "set", "list", "quit"), NullCompleter.INSTANCE);
            ...
            try {
                ...
                if (ss.length == 1 && any(cmd, "list", "quit")) {
                    ...
                    if ("check".equalsIgnoreCase(cmd)) {
                        System.out.println(JSON.toJSONString(switcherService.check(ss[1]), true));
                    } else if ("on".equalsIgnoreCase(cmd)) {
                        switcherService.on(ss[1]);
                    } else if ("off".equalsIgnoreCase(cmd)) {
                        switcherService.off(ss[1]);
                    }
                } else if (ss.length == 3 && any(cmd, "set")) {
                    switcherService.set(ss[1], Boolean.valueOf(ss[2]));
                }
